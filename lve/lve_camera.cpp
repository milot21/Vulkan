...
namespace lve {
    ...
    void LveCamera::setViewDirection(glm::vec3 position, glm::vec3 direction, glm::vec3 up) {
        //set up an orthonormal basis, that consists of three vectors, length =1 orthogonal ->90 degree angles
    ...
    // use setViewDirection, direction being given from cameras position to the target objects positions
    void LveCamera::setViewTarget(glm::vec3 position, glm::vec3 target, glm::vec3 up) {
        setViewDirection(position, target - position, up);
    }

    /** transforms any vertex from world space to view space,
     * camera becomes the origin
     * cam looks down the -Z axis
     * the world moves around the camera
     * @param position
     * @param rotation
     */
